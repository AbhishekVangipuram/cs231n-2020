...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
      prev_h, tmpcache = rnn_step_forward(x[:,t,:], prev_h, Wx, Wh, b)
      h[:, t, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    t = T-1
    while t >= 0:
      dx_, dh0_, dWx_, dWh_, db_ = rnn_step_backward(dh[:,t,:] + dh0, cache.pop())
      dx[:,t,:] = dx_
      dh0 = dh0_
      dWx += dWx_
      dWh += dWh_
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
      prev_h, prev_c, tmpcache = lstm_step_forward(x[:,t,:], prev_h, prev_c, Wx, Wh, b)
      h[:, t, :] = prev_h
      cache.append(tmpcache)
...
def lstm_backward(dh, cache):
    ...
    t = T-1
    while t >= 0:
      dx_, dh0_, dc, dWx_, dWh_, db_ = lstm_step_backward(dh[:,t,:] + dh0, dc, cache.pop())
      dx[:,t,:] = dx_
      dh0 = dh0_
      dWx += dWx_
      dWh += dWh_
