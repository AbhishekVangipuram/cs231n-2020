...
def compute_saliency_maps(X, y, model):
    ...
    grad  = torch.FloatTensor([1.0]*scores.shape[0])
    scores.backward(gradient = grad)
...
def make_fooling_image(X, target_y, model):
    ...
    while y != target_y:
      ...
      # only do the backward pass for target_y
      ...
      # zero out gradients of input image for next step
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    # generate scores
    scores = model(img)
    ideal = torch.FloatTensor([0.0]*scores.shape[1])
    ideal[target_y] = 1.0
    # L2 Regularization for class target_y
