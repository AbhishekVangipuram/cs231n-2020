...
def content_loss(content_weight, content_current, content_original):
    ...
    return content_weight * torch.sum(torch.square(torch.sub(F, P)))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      ...
      A = style_targets[i]
      style_loss += style_weights[i] * torch.sum(torch.square(torch.sub(G,A)))
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    row_diffs = img[:,:,1:,:] - img[:,:,:-1,:]
    column_diffs = img[:,:,:,1:] - img[:,:,:,:-1]
    return tv_weight * (row_diffs.square().sum() + column_diffs.square().sum())
