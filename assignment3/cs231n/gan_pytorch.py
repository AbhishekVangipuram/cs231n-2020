...
def sample_noise(batch_size, dim, seed=None):
    ...
    return -2 * torch.rand(size=(batch_size,dim)) + 1
...
def discriminator_loss(logits_real, logits_fake):
    ...
    sz = logits_real.size()
    loss = bce_loss(logits_real, torch.ones(sz).type(dtype)) + bce_loss(logits_fake, torch.zeros(sz).type(dtype))
...
def generator_loss(logits_fake):
    ...
    sz = logits_fake.size()
    loss = bce_loss(logits_fake, torch.ones(sz).type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    sz = scores_real.size()
    loss = 0.5 * torch.mean((scores_real - torch.ones(sz).type(dtype))**2) + 0.5 * torch.mean(scores_fake**2)
...
def ls_generator_loss(scores_fake):
    ...
    sz = scores_fake.size()
    loss = 0.5 * torch.mean((scores_fake-torch.ones(sz).type(dtype))**2)
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
                Unflatten(N=batch_size, C=1, H=28, W=28),           # (N,1,28,28)
                nn.Conv2d(1, 32, kernel_size = 5, stride=1),        # (N,32,24,24)
                nn.LeakyReLU(),
                nn.MaxPool2d(kernel_size=(2,2), stride = (2,2)),    # (N,32,12,12)
                nn.Conv2d(32, 64, kernel_size = 5, stride=1),       # (N,64,8,8)
                nn.LeakyReLU(),
                nn.MaxPool2d(kernel_size=(2,2), stride=(2,2)),      # (N,64,4,4)
                Flatten(),
